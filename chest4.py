...
temp_dataframe = pd.DataFrame({'class_weights': temp_weights, 
                                ...
                                'custom_weight_products': custom_weight_products})
temp_dataframe
...
# %%
proposed_split = (len(df)*0.30)
# %%
proposed_split / len(df.class_id.unique())
# %%
print(len(df.class_id.unique()))
# %%
def prepare_class_split(dataframe, target="class_name", p_split=0.20, test_target_split=0.50, verbose=False, helpers="dataframe"):
...
# %%
test = prepare_class_split(df, target="class_name", p_split=0.30, test_target_split=0.50, verbose=True, helpers="dataframe")
...
def training(model, train_dataloader, num_epochs):
    ...
    weight = torch.FloatTensor(pos_contribution).to(device)
...
#create sns barplot of value of given columns in a dataframe
# %%
def show_barplot(df, columns):
    sns.barplot(x=df.class_name, y=df[columns])
# %%
show_barplot(temp_dataframe, ['sum_of_class_names', 'weight_products', 'custom_weight_products'])
# %%
plt.xticks(rotation=90)
sns.barplot(x=temp_dataframe.class_name, y=temp_dataframe['sum_of_class_names'])
# %%
plt.xticks(rotation=90)
sns.barplot(x=temp_dataframe.class_name, y=temp_dataframe['weight_products'])
# %%
plt.xticks(rotation=90)
sns.barplot(x=temp_dataframe.class_name, y=temp_dataframe['custom_weight_products'])
# %%
